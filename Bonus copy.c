...
float function(struct node* head, struct node cache[cache_size], int cache_size, int set_size, int type){
    ...
    while(temp->next != NULL){
        ...
        int bool = 0, low = (temp->data % (cache_size / set_size)), high = (temp->data % (cache_size / set_size)) + set_size;
        ...
        if(!bool){
            if(type == 0){
                int index;
                int low = (temp->data % (cache_size / set_size)), high = (temp->data % (cache_size / set_size)) + set_size;
